 ...
 * */
if ( ! function_exists( 'vc_fl_portfolio_info_function' ) ) {
    function vc_fl_portfolio_info_function($atts, $content = null)
    {
        $css_classes[] = 'fl-li-portfolio-info cf';

        global $fl_helping_responsive_style;

        $atts = vc_map_get_attributes('vc_fl_portfolio_info', $atts);
        ...
        $result .= '<div class="fl-right-content fl-text-regular-style">' . wpb_js_remove_wpautop($content, true) . '</div>';
...
add_shortcode('vc_fl_portfolio_info', 'vc_fl_portfolio_info_function');

<?php
//
require_once('widgets_files/footer_contact_info_widget.php');
require_once('widgets_files/about_us_widget.php');
require_once('widgets_files/popular_post_widget.php');
require_once('widgets_files/social_profiles_widget.php');
require_once('widgets_files/wpml_change_languages_widget.php');
require_once('widgets_files/footer_logotype_widget.php');
require_once('widgets_files/header_contact_info.php');
require_once('widgets_files/header_social_widget.php');
// Register Widgets
function fl_init_widgets() {
    // Contact Info
    register_widget( 'FL_THEME_HELPER_Contact_Info' );
    // About Us
    register_widget( 'FL_THEME_HELPER_About_Us' );
    // Social Profiles
    register_widget( 'FL_THEME_HELPER_Social_Profiles' );
    // Popular Post
    register_widget( 'FL_THEME_HELPER_Popular_Post' );
    // WPML Change Lang
    register_widget( 'FL_THEME_HELPER_WMPL_Change_Languages' );
    // Footer Logotype
    register_widget( 'FL_THEME_HELPER_Footer_Logotype' );
    // Header Contact Info
    register_widget( 'FL_THEME_HELPER_Header_Contact_info' );
    // Header Contact Info
    register_widget( 'FL_THEME_HELPER_Header_Social' );
...
class SB_WP_Widget extends WP_Widget {
    ...
    public function __construct() {
        ...
        add_action('admin_enqueue_scripts', array($this, 'fl_register_scripts'),100);
        add_action('wp_enqueue_scripts', array($this,'fl_register_helper_scripts'));
    }

    function fl_register_scripts() {
        wp_enqueue_script('wp-color-picker');
        wp_enqueue_style('wp-color-picker');

    }

    function fl_register_helper_scripts() {
        wp_enqueue_style('custom-changer-lang-css', plugin_dir_url( __FILE__ ). 'assets/css/change_language.css', array(),'1.0');
        wp_enqueue_script('custom-changer-lang-js',plugin_dir_url( __FILE__ ).'assets/js/change_language.js', false, null , true);
    ...
    protected function _input_wp_dropdown_categories($name) {
        ...
        echo '<option value="">'.esc_html__('All categories' , 'fl-themes-helper').'</option>';
    ...
    protected function _input_colorpicker($name) {
        ...
        echo '<p>';
        echo '<label for="'. esc_attr($input_id) .'">'.$label.'</label>';
        echo '<input type="text" class="fl-widget-colorpicker-field" id="'. esc_attr($input_id) .'" name="'.esc_attr($input_name).'" value="'.esc_attr($value).'" /></label>';
    ...
    protected function _input_upload_image($name) {
        ...
        <p>
            ...
            <input class="upload_image_button button" type="button" value="<?php esc_html_e('Upload Image','fl-themes-helper'); ?>" />
            <?php if(filter_var($value, FILTER_VALIDATE_URL)) : ?>
                <input class="remove_image_button button" type="button" value="<?php esc_html_e('Remove Image','fl-themes-helper'); ?>" />
            <?php endif; ?>
